...
curl -k "https://'${splunk_server_and_port}'/services/collector" \
    ...
    -d '{"event":"metric","fields": {"metric_name:load1":"'${load1}'","metric_name:load5":"'${load5}'","metric_name:load15":"'${load15}'"},"index":"'${splunk_index}'","host":"'${HOSTNAME}'", "sourcetype": "loadaverage", "source":"/proc/loadavg"}'
